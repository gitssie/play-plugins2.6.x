...
trait Realm {
  ...
  /**
    ...
    */
    def getAuthenticationInfo(token: AuthenticationToken,passwordService: PasswordService): Either[Subject,String]

    def getRoles(subject: Subject):Seq[Role]

    def getPermissions(subject: Subject):Seq[Permission]
}